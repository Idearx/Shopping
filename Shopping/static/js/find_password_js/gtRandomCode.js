...
window.gtRamdonCodeObj = {
	...
	{
		var captchaInfo = $("#captchaInfo");
		var callback = localInfo.gtbind_callback;
		if(callback){
			callback(captchaObj);
		}else{
			var captchaProduct = captchaInfo.attr("captchaProduct");
			if(captchaProduct == 'float'){
				gtRamdonCodeObj.callback_float(captchaObj);
			}else{
				gtRamdonCodeObj.callback_login(captchaObj);
			}
		}
	},
	
	callback_float : function(captchaObj){
	    ...
	    $(".dinput-UI6").css("padding","0");
	    $(".dinput-UI6").css("border-radius","0");
	...
	callback_login : function(captchaObj){
		if (localInfo.displayCaptchaType == "1"){
			$('#captcha1').on('click', function () {
				captchaObj.verify();
				window.captchaObj = captchaObj;
			});
		}
		...
		captchaObj.onSuccess(function () {
			...
			if(localInfo.currentPage == "SMSLogin"){
				getSMSCodeV3();
			}else{
				login();
			}
		...
		captchaObj.onReady(function(){
			localInfo.gtLoadReady = true;
			$('#captcha1').trigger("gtLoadSuccess");
		});
