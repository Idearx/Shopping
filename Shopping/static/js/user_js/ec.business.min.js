...
ol.load.define("jquery", [{
    ...
    type: "js"
}, {uri: "base/jquery-migrate-1.4.1.min.js", type: "js"}, {
    uri: "base/jquery-migrate-3.0.1.min.js",
    type: "js"
}, {uri: "base/helper.min.js", type: "js"}]);
ol.load.define("jquery.form", ["jquery", {
    mark: "jquery.form",
    uri: "base/jquery.form-4.2.2.js",
    type: "js",
    charset: "utf-8",
    depend: true
}]);
...
if ("object" !== typeof ol.box && "function" !== typeof ol.box) {
    ol.load.define("ec.box", ["jquery", {uri: "ec.box/box-min.js", type: "js", depend: true}])
}
    ...
    if (referrer) {
        ...
        $.each(cps_id_array, function (index, item) {
            if (item.indexOf(domain) == 0) {
                ...
                ec.util.cookie.set("cps_id", cid, {expires: 3, domain: "vmall.com"});
                return
            }
        })
    }
...
ec.code.saveCpsInfoToCookie = function () {
    ...
    if (wi.length > 0 && wi.length < 200) {
        ec.util.cookie.set("cps_wi", wi, {expires: 3, domain: "vmall.com"})
    }
};
...
ec.code.addService = function (options) {
    $(function () {
        if (options.showService) {
            ...
            $("#tools-nav-service-robotim").attr("href", options.live800Url4Web + "&enterurl=" + encodeURIComponent(window.location.href) + "&k=1&from=06&remark=" + encodeURIComponent(memo)).css("display", "block");
            $("#tools-nav-service-robotim-button").attr("href", options.live800Url4Web + "&enterurl=" + encodeURIComponent(window.location.href) + "&k=1&from=06&remark=" + encodeURIComponent(memo)).css("display", "block")
        }
...
(function () {
    ...
    try {
        ...
        if (uri == domainMain || uri == domainMain + "/index.html" || uri == domainMain + "/" || isProDetail || isHonor || isHuawei || isRecyle || isNotice || isList) {
            if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent)) {
                var redirectWap = ec.util.cookie.get("redirectWap");
                if (!redirectWap == "1" && !isProDetail) {
                    if (isHonor) {
                        ...
                        var wapUri = domainWap + uri.substring(uri.indexOf("/notice"));
                        location.href = wapUri
                    } else if (isList) {
                        var wapUri = domainWap + "/category/detail" + uri.substring(uri.indexOf("/list") + 5) + uri.substring(uri.indexOf("/list") + 5);
                        location.href = wapUri
                    } else {
...
ec.binding.allCount = function () {
    ...
    if (allCountWechat > 0) {
        $("#li-order-small span").html("小程序订单<em>" + allCountWechat + "</em>")
    }
...
ec.ready(function () {
    ec.code.addAnalytics({baidu: true});
